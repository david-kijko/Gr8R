...
fn agent_explain(args: &[String]) -> std::io::Result<i32> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "help" | "--help" | "-h" => {
                eprintln!("usage: gr8r agent explain <target> [--json|--verbose]");
                eprintln!("usage: gr8r agent explain --file PATH --agent LABEL [--json|--verbose]");
                return Ok(0);
            }
            ...
            value => {
                if target.is_some() {
                    eprintln!("usage: gr8r agent explain <target> [--json]");
                    return Ok(2);
                }
    ...
    let explain = if let Some(path) = file {
        if target.is_some() {
            eprintln!("usage: gr8r agent explain --file PATH --agent LABEL [--json]");
            return Ok(2);
        }
        let Some(agent_label) = agent else {
            eprintln!("gr8r agent explain --file requires --agent LABEL");
            return Ok(2);
        };
...
fn agent_start(args: &[String]) -> std::io::Result<i32> {
    let Some(name) = args.first() else {
        eprintln!("usage: gr8r agent start <name> [--cwd PATH] [--workspace ID] [--tab ID] [--split right|down] [--env KEY=VALUE] [--focus|--no-focus] -- <argv...>");
        return Ok(2);
    };
...
fn agent_list(args: &[String]) -> std::io::Result<i32> {
    if !args.is_empty() {
        eprintln!("usage: gr8r agent list");
        return Ok(2);
    }
...
fn agent_get(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: gr8r agent get <target>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gr8r agent get <target>");
...
fn agent_focus(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: gr8r agent focus <target>");
        return Ok(2);
    };
    if args.len() != 1 {
        eprintln!("usage: gr8r agent focus <target>");
...
fn agent_attach(args: &[String]) -> std::io::Result<i32> {
    let (target, takeover) =
        match super::parse_attach_target(args, "usage: gr8r agent attach <target> [--takeover]") {
            Ok(parsed) => parsed,
...
fn agent_wait(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: gr8r agent wait <target> --status <idle|working|blocked|unknown> [--timeout MS]");
        return Ok(2);
    };
...
fn agent_rename(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: gr8r agent rename <target> <name>|--clear");
        return Ok(2);
    };
    if args.len() < 2 {
        eprintln!("usage: gr8r agent rename <target> <name>|--clear");
...
fn agent_send(args: &[String]) -> std::io::Result<i32> {
    if args.len() < 2 {
        eprintln!("usage: gr8r agent send <target> <text>");
        return Ok(2);
    }
...
fn agent_read(args: &[String]) -> std::io::Result<i32> {
    let Some(target) = args.first() else {
        eprintln!("usage: gr8r agent read <target> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
        return Ok(2);
    };
...
fn print_agent_help() {
    eprintln!("gr8r agent commands:");
    eprintln!("  gr8r agent list");
    eprintln!("  gr8r agent get <target>");
    eprintln!("  gr8r agent read <target> [--source visible|recent|recent-unwrapped] [--lines N] [--format text|ansi] [--ansi]");
    eprintln!("  gr8r agent send <target> <text>");
    eprintln!("  gr8r agent rename <target> <name>|--clear");
    eprintln!("  gr8r agent focus <target>");
    eprintln!("  gr8r agent wait <target> --status <idle|working|blocked|unknown> [--timeout MS]");
    eprintln!("  gr8r agent attach <target> [--takeover]");
    eprintln!("  gr8r agent start <name> [--cwd PATH] [--workspace ID] [--tab ID] [--split right|down] [--env KEY=VALUE] [--focus|--no-focus] -- <argv...>");
    eprintln!("  gr8r agent explain <target> [--json]");
    eprintln!("  gr8r agent explain --file PATH --agent LABEL [--json]");
